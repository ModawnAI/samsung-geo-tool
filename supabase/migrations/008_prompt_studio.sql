-- Migration: 008_prompt_studio.sql
-- Prompt Studio: Stage-based prompt management and testing system
-- Apply via: Supabase Dashboard > SQL Editor

-- ============================================================================
-- Stage Prompts Table
-- Stores stage-specific prompt configurations linked to prompt versions
-- ============================================================================
CREATE TABLE IF NOT EXISTS stage_prompts (
  ...
  stage VARCHAR(30) NOT NULL CHECK (stage IN (
    'description', 'usp', 'faq', 'chapters',
    'case_studies', 'keywords', 'hashtags'
  )),

  -- Stage-specific system prompt (overrides base prompt)
...
-- Records individual test executions for each stage prompt
-- ============================================================================
...
-- Stores reusable test input configurations for each stage
-- ============================================================================
...
CREATE POLICY "Users can view all stage prompts"
  ON stage_prompts FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert their own stage prompts"
  ON stage_prompts FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Users can update their own stage prompts"
  ON stage_prompts FOR UPDATE
  TO authenticated
  USING (auth.uid() = created_by);

CREATE POLICY "Users can delete their own stage prompts"
  ON stage_prompts FOR DELETE
  TO authenticated
  USING (auth.uid() = created_by);

-- Test Runs Policies
CREATE POLICY "Users can view all test runs"
  ON prompt_test_runs FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert their own test runs"
  ON prompt_test_runs FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Users can update their own test runs"
  ON prompt_test_runs FOR UPDATE
  TO authenticated
  USING (auth.uid() = created_by);

-- Test Inputs Policies
CREATE POLICY "Users can view shared or own test inputs"
  ON stage_test_inputs FOR SELECT
  TO authenticated
  USING (is_shared = true OR auth.uid() = created_by);

CREATE POLICY "Users can insert their own test inputs"
  ON stage_test_inputs FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Users can update their own test inputs"
  ON stage_test_inputs FOR UPDATE
  TO authenticated
  USING (auth.uid() = created_by);

CREATE POLICY "Users can delete their own test inputs"
  ON stage_test_inputs FOR DELETE
  TO authenticated
  USING (auth.uid() = created_by);
